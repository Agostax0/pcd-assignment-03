...
object ActorReceptionistMessages:
  ...
  case class GetActors(nameShared: String, replyTo: ActorRef[ActorReceptionistResponses]) extends ActorReceptionistMessages
...
object BoidActorsReceptionist:
  def apply(
    ...
    Behaviors.receiveMessage {
      case Register(name, ref: ActorRef[BoidActorMessages]) => apply(db :+ (name, ref))
      ...
      case GetActors(nameShared, replyTo) =>
        val res = db.filter(_._1 contains nameShared)
